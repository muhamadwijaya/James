...
from .settings_model import defaults, VERSION, LEVELS
...
class SettingsPage(PageBase):
    ...
    def build_devices(self):
        ...
        for level,x in [('BOX',110),('CARTON',224)]:
            key='scanners.'+level+'.';ref=defaults()['scanners'][level]
            self.control(key+'device',x,529,105)
            widget=self.control(key+'port',x,562,105,'editable',list(dict.fromkeys([ref['port'],'KEYBOARD']+ports)))
            setattr(self,'scanner_'+level.lower(),widget)
            self.control(key+'trigger',x,595,105,'combo',['AUTO','MANUAL'])
            self.control(key+'autofocus',x,628,105,'toggle')
            self.control(key+'exposure',x,661,105,'int',minimum=1,maximum=10000).setSuffix(' ms')
            self.control(key+'resolution',x,694,105,'combo',['640 x 480','1280 x 720','1920 x 1080'])
        self.control('camera.device',443,529,169)
        self.camera_ip=self.control('camera.address',443,562,169)
        self.camera_ip.setToolTip('Alamat IP/URL snapshot atau ID kamera USB. Pilih perangkat melalui Kalibrasi Kamera.')
        self.control('camera.trigger',443,595,169,'combo',['AUTO','MANUAL'])
        self.control('camera.autofocus',443,628,169,'toggle')
        self.control('camera.exposure',443,661,169,'int',minimum=1,maximum=10000).setSuffix(' ms')
        self.control('camera.resolution',443,694,169,'combo',['640 x 480','1280 x 720','1920 x 1080'])
        self.camera_preview=QLabel(self);self.camera_preview.setGeometry(443,731,169,114);self.camera_preview.setAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def apply_appearance(self,cfg):
        ...
        for key,control in self.controls.items():
            compact=' QComboBox,QLineEdit{font-size:10px;padding-left:3px;padding-right:3px;} QComboBox::drop-down{width:14px;}' if key.startswith('printers.') or key.endswith('.resolution') or key.endswith('.device') or key=='mode' else ''
    ...
    def paint_content(self):
        ...
        for label,y in [('DEVICE NAME',529),('KONEKSI (COM)',562),('TRIGGER MODE',595),('AUTOFOCUS',628),('EXPOSURE',661),('RESOLUSI',694),('PREVIEW',733)]:self.text(28,y,80,27,label,10,MUTED)
        for label,y in [('DEVICE NAME',529),('KONEKSI (IP / URL)',562),('TRIGGER MODE',595),('AUTOFOCUS',628),('EXPOSURE',661),('RESOLUSI',694),('PREVIEW',733)]:self.text(349,y,91,27,label,10,MUTED)
        for level,x in [('BOX',110),('CARTON',224)]:
            self.rect(x,731,105,114,'#112936','#041d2c','#426176');self.icon('scanner',x+34,756,37)
            value=self.runtime.status.get('scanner_'+level,{}).get('status','BELUM DITES');self.text(x+4,806,97,26,value,9,MUTED,align=Qt.AlignmentFlag.AlignCenter,wrap=True)
        self.rect(443,731,169,114,'#112936','#041d2c','#426176')
        if self.camera_preview.pixmap().isNull():self.icon('camera',500,764,46)
